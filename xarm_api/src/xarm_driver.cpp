...
{   
    ...
    void XArmDriver::_xarm_gripper_action_execute(float position)
    {
    ...
    void XArmDriver::send_gripper_command(float position)
    {
        std::thread{BIND_CLS_CB_1(&XArmDriver::_xarm_gripper_action_execute), position}.detach();
    }
    ...
    void XArmDriver::_bio_gripper_action_execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<control_msgs::action::ParallelGripperCommand>> goal_handle)
    {
        ...
        while (is_move && rclcpp::ok())
        {
            ...
            if (ret == 0) {
                ...
                bio_gripper_feedback_->state = bio_gripper_joint_state_msg_;
                try {

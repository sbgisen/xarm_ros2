...
void UFRobotSystemHardware::_init_ufactory_driver(void) {
  ...
  RCLCPP_INFO(
      LOGGER,
      "[%s] dof: %d, velocity_control: %d, add_gripper: %d, add_bio_gripper: "
      "%d, baud_checkset: %d, default_gripper_baud: %d",
      robot_ip_.c_str(), dof, velocity_control_, add_gripper, add_bio_gripper,
...
bool UFRobotSystemHardware::_xarm_is_ready_write(void) {
  ...
  if (curr_state > 2) {
    if (last_state != curr_state) {
      ...
      RCLCPP_WARN(LOGGER, "[%s] Robot State detected! State: %d",
  ...
  if (!(velocity_control_ ? curr_mode == XARM_MODE::VELO_JOINT
    ...
    if (last_mode != curr_mode) {
      last_mode = curr_mode;
      RCLCPP_WARN(LOGGER, "[%s] Robot Mode detected! Mode: %d",
...
bool UFRobotSystemHardware::_need_reset() {
  ...
  if (!write_succeed) {
    ...
    // RCLCPP_ERROR(LOGGER, "[%s] Write() failed, failed_ret=%d !, Setting Robot
    // State to STOP... (ret: %d)", robot_ip_.c_str(), write_code_, ret);
    RCLCPP_ERROR(LOGGER, "[%s] Write() failed, failed_ret=%d !",
                 robot_ip_.c_str(), write_code_);
